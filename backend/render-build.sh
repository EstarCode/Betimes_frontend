...
if [ -z "$(printf '%s' "$DATABASE_URL" | tr -d '[:space:]')" ]; then
  echo "❌ DATABASE_URL is empty or missing. In Render, set it from the PostgreSQL service connection string."
  exit 1
fi
...
case "$DATABASE_URL" in
  postgres://*|postgresql://*)
    ...
    ;;
  *)
    echo "❌ DATABASE_URL must start with postgres:// or postgresql://"
    exit 1
    ;;
esac
